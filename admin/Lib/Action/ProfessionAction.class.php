...
class ProfessionAction extends BaseAction
{
	...
	function add()
	{
		if(isset($_POST['dosubmit'])){
			...
			if($result){
				...
				if( $cate['pid']!=0 ){
					M('Profession_cate')->where("id=".$cate['pid'])->setInc('article_nums');
					M('Profession_cate')->where("id=".$data['cate_id'])->setInc('article_nums');
				}else{
					M('Profession_cate')->where("id=".$data['cate_id'])->setInc('article_nums');
				}
				$this->success('添加成功');
	...
	function delete()
    {
		...
		if( isset($_POST['id'])&&is_array($_POST['id']) ){
			...
			foreach( $_POST['id'] as $val ){
				...
				$cate = M('Profession_cate')->field('id,pid')->where("id=".$article['cate_id'])->find();
				if( $cate['pid']!=0 ){
					M('Profession_cate')->where("id=".$cate['pid'])->setDec('article_nums');
					M('Profession_cate')->where("id=".$article['cate_id'])->setDec('article_nums');
				}else{
					M('Profession_cate')->where("id=".$article['cate_id'])->setDec('article_nums');
				}
	...
	function status()
	{
		...
		$sql 	= "update ".C('DB_PREFIX')." profession set $type=($type+1)%2 where id='$id'";
