...
class InstituteCateAction extends BaseAction
{

	//分类列表
    function index()
    {
        $article_cate_mod = M('Institute_cate');
...
//    	    	$val['nums'] =$nums;
    ...
    private function _cate_exists($name, $pid, $id=0)
    {
        ...
        $result = M('Institute_cate')->where($where)->count();
	...
	function status()
	{
		...
		$sql 	= "update ".C('DB_PREFIX')."Institute_cate set $type=($type+1)%2 where id='$id'";
